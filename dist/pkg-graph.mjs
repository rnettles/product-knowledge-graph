...
function createNode(value2, tagName, ctx) {
  ...
  const node = tagObj?.createNode ? tagObj.createNode(ctx.schema, value2, ctx) : typeof tagObj?.nodeClass?.from === "function" ? tagObj.nodeClass.from(ctx.schema, value2, ctx) : new Scalar(value2);
  if (tagName)
    node.tag = tagName;
...
function collectionFromPath(schema4, path, value2) {
  let v = value2;
...
function resolveAliasValue(ctx, value2) {
  return ctx && isAlias(value2) ? value2.resolve(ctx.doc, ctx) : value2;
}

// node_modules/yaml/browser/dist/nodes/addPairToJSMap.js
function addPairToJSMap(ctx, map2, { key, value: value2 }) {
  if (isNode(key) && key.addToJSMap)
    key.addToJSMap(ctx, map2, value2);
  else if (isMergeKey(ctx, key))
    addMergeToJSMap(ctx, map2, value2);
  else {
...
function stringifyBlockCollection({ comment, items }, ctx, { blockItemPrefix, flowChars, itemIndent, onChompKeep, onComment }) {
  ...
  for (let i = 0; i < items.length; ++i) {
    ...
    chompKeep = false;
    let str2 = stringify(item2, itemCtx, () => comment2 = null, () => chompKeep = true);
    if (comment2)
...
function stringifyFlowCollection({ items }, ctx, { flowChars, itemIndent }) {
  ...
  for (let i = 0; i < items.length; ++i) {
    ...
    if (comment)
      reqNewline = true;
    let str = stringify(item2, itemCtx, () => comment = null);
...
var Document = class _Document {
  ...
  createNode(value2, replacer, options) {
    ...
    const node = createNode(value2, tag, ctx);
  ...
  setIn(path, value2) {
    if (isEmptyPath(path)) {
      ...
      this.contents = collectionFromPath(this.schema, Array.from(path), value2);
    } else if (assertCollection(this.contents)) {
...
function resolveBlockSeq({ composeNode: composeNode2, composeEmptyNode: composeEmptyNode2 }, ctx, bs, onError, tag) {
  ...
  for (const { start, value: value2 } of bs.items) {
    ...
    const node = value2 ? composeNode2(ctx, value2, props, onError) : composeEmptyNode2(ctx, props.end, start, null, props, onError);
    if (ctx.schema.compat)
...
// src/model.js
var CLASSES = /* @__PURE__ */ new Set(["contradiction", "absence", "inference", "assessment"]);
var SEVERITIES = /* @__PURE__ */ new Set(["error", "diagnostic", "metric"]);
function finding(code, severity, klass, message, details = {}) {
...
function list(value2) {
  ...
  return Array.isArray(value2) ? value2.filter((v) => v != null && v !== "") : [value2];
}
function text(value2) {
  if (value2 == null) return "";
  if (typeof value2 === "object") return String(value2.display ?? value2.path ?? value2.value ?? "").trim();
  return String(value2).replace(/^\[\[/, "").replace(/\]\]$/, "").split("|")[0].trim();
}

// src/profile.js
var versionParts = (value2) => String(value2 ?? "0.0.0").split(".").map((part) => Number.parseInt(part, 10) || 0);
var compareVersions = (a, b) => {
  const left = versionParts(a), right = versionParts(b);
  for (let i = 0; i < 3; i++) if (left[i] !== right[i]) return left[i] - right[i];
  return 0;
};
var supports = (range, version) => String(range).split(/\s+/).filter(Boolean).every((term) => {
...
function compileProfile(profile) {
  ...
  if (findings.some((item2) => item2.severity === "error")) return { profile, findings, valid: false };
  const binding = (role) => {
    const value2 = profile.host_bindings?.[role];
    return typeof value2 === "string" ? value2 : value2?.property;
  };
  return {
    valid: true,
    findings: [],
...
function normalizeRecord(source, profile, sourceId = source.sourceId ?? source.path ?? source.file?.path ?? "<unknown>") {
  const ids2 = list(source.pkg_ids).map(text).filter(Boolean);
  const known = new Set([
    "pkg_ids",
    "pkg_artifact_kind",
    "pkg_authority_status",
    "pkg_subject",
    "pkg_collection",
    "pkg_parent",
    "pkg_facets",
    "pkg_superseded_by",
    "pkg_implementation_status",
    "pkg_implementation_source",
    "pkg_realizes",
    "pkg_depends_on",
    "pkg_invariants_declared",
    "pkg_invariants_referenced",
    profile.binding("title"),
    profile.binding("owner"),
    profile.binding("last_reviewed"),
    profile.binding("review_after")
  ].filter(Boolean));
  return {
    source,
    sourceId,
    path: source.path ?? source.file?.path,
...
function normalizeRecords(sources, profile) {
  return sources.map((source, index) => normalizeRecord(source, profile, source.sourceId ?? source.path ?? source.file?.path ?? `<record:${index}>`));
}

// src/graph.js
var add = (map2, key, value2) => {
  if (!map2.has(key)) map2.set(key, []);
  map2.get(key).push(value2);
};
function buildGraph(records, profile) {
  const findings = [];
  const byId = /* @__PURE__ */ new Map(), claims = /* @__PURE__ */ new Map();
  ...
  for (const [id, owners] of claims) {
    if (owners.length === 1) byId.set(id, owners[0]);
      ...
      artifactIds: [id],
      sourceIds: owners.map((x) => x.sourceId),
      detail: id
    }));
  }
  const resolve = (input) => {
  ...
  const indexes = {
    ...
    byAuthorityStatus: /* @__PURE__ */ new Map(),
    byImplementationStatus: /* @__PURE__ */ new Map(),
    byFacet: /* @__PURE__ */ new Map()
  };
  for (const record of records) {
    ...
    for (const ref of record.parentRefs) {
      const r = resolve(ref);
      if (r.target) add(indexes.structuralChildren, r.target.sourceId, record);
    }
    for (const ref of record.subjectRefs) {
      const r = resolve(ref);
      if (r.target) add(indexes.ownedBySubject, r.target.sourceId, record);
    }
    ...
    for (const ref of record.dependencyRefs) {
      const r = resolve(ref);
      if (r.target) add(indexes.inverseDependsOn, r.target.sourceId, record);
    }
...
function closure(graph, seed, inverseIndex) {
  const start = typeof seed === "string" ? graph.resolve(seed).target : seed;
  if (!start) return [];
  const seen = /* @__PURE__ */ new Set([start.sourceId]), queue = [start], out = [];
  while (queue.length) {
...
function detectCycles(records, refsOf, resolve) {
  const cycles = [];
  const done = /* @__PURE__ */ new Set();
  const visit3 = (record, stack = []) => {
    const at = stack.findIndex((item2) => item2.sourceId === record.sourceId);
    if (at >= 0) {
      cycles.push([...stack.slice(at), record]);
      return;
    }
    if (done.has(record.sourceId)) return;
    for (const ref of refsOf(record)) {
      const target = resolve(ref).target;
      if (target) visit3(target, [...stack, record]);
    }
    done.add(record.sourceId);
  };
  for (const record of records) visit3(record);
...
// src/lint/rules.js
var ids = (record) => record.ids.length ? record.ids : [record.sourceId];
var item = (code, message, record, detail = message, severity = "error", klass = "contradiction") => finding(code, severity, klass, message, { artifactIds: ids(record), sourceIds: [record.sourceId], detail });
function lintCore(graph) {
  ...
  for (const [name, refsOf, code] of [
    ...
    artifactIds: cycle.flatMap(ids),
    sourceIds: cycle.map((x) => x.sourceId),
    detail: [...new Set(cycle.map((x) => x.sourceId))].sort().join("|")
  }));
  for (const [id, declarers] of graph.indexes.invariantDeclarations) if (declarers.length > 1) out.push(finding("CORE-INV-001", "error", "contradiction", `Invariant ${id} has ${declarers.length} declarations`, {
    artifactIds: [id],
    sourceIds: declarers.map((x) => x.sourceId),
    detail: id
  }));
  for (const [id, references] of graph.indexes.invariantReferences) if ((graph.indexes.invariantDeclarations.get(id) ?? []).length !== 1) out.push(finding("CORE-INV-002", "error", "contradiction", `Referenced invariant ${id} does not resolve to exactly one declaration`, {
    artifactIds: [id],
    sourceIds: references.map((x) => x.sourceId),
    detail: id
  }));
  return out;
}
function lintProject(graph) {
  const out = [];
  ...
  for (const record of graph.records) {
    ...
    for (const [facetName, raw] of Object.entries(record.facets)) {
      ...
      if (!spec) {
        out.push(item("PROJECT-FACET-001", `Unknown facet: ${facetName}`, record, facetName));
        continue;
      }
      ...
      for (const value2 of values) if (spec.values && !spec.values.includes(value2)) out.push(item("PROJECT-FACET-003", `Unknown ${facetName} value: ${value2}`, record, `${facetName}:${value2}`));
    }
    ...
    if (p.projection?.path_template && record.path) {
      const expected = p.projection.path_template.replaceAll("{collection}", record.collection).replaceAll("{subject}", record.subjectRefs[0] ?? "").replaceAll("{kind}", record.kind).replaceAll("{id}", record.ids[0] ?? "");
...
function lintDiagnostics(graph, now = /* @__PURE__ */ new Date()) {
  const out = [];
  for (const record of graph.records) {
...
// src/reports/estate-health.js
function estateHealth(graph, findings = []) {
...
var identity = (record) => record.ids[0] ?? record.sourceId;
var label = (record) => record.title || identity(record);
function subjects(graph) {
...
function evaluateEvidence(evidence = {}, artifacts = []) {
...
// src/reports/structure-explorer.js
function structureExplorer(graph) {
  ...
  const node = (record, trail = /* @__PURE__ */ new Set()) => {
    if (trail.has(record.sourceId)) return { id: identity(record), label: label(record), cycle: true };
    const next = /* @__PURE__ */ new Set([...trail, record.sourceId]);
    return {
      id: identity(record),
      label: label(record),
...
var inferredPlacements = (graph, config) => {
  ...
  for (const record of graph.records) {
    ...
    const candidates = graph.records.filter((candidate) => candidate !== record && candidate.kind === expectedKind && identityKey && key(candidate) === identityKey);
    if (candidates.length !== 1) continue;
    rows.push({
      artifactId: identity(record),
      artifactPath: record.path,
      inferredParentId: identity(candidates[0]),
      inferredParentPath: candidates[0].path,
      expectedKind,
      identityKey,
      class: "inference",
      authority: "display-only",
      provenance: "configured-identity-key"
    });
  }
  return rows.sort((a, b) => a.artifactPath.localeCompare(b.artifactPath));
};
function traceabilityExplorer(graph, config = graph.profile.reports.traceability ?? {}) {
  ...
  return {
    report: "traceability-explorer",
    expectedChildren: expected,
    inferredPlacements: inferredPlacements(graph, config),
    artifacts
  };
}

// src/reports/coverage.js
var summarize = (records) => records.map((x) => ({ id: identity(x), label: label(x), kind: x.kind, subjectRefs: x.subjectRefs, implementationStatus: x.implementationStatus }));
function intentCoverage(graph, seed, config = {}) {
...
// src/reports/review-freshness.js
function reviewFreshness(graph, now = /* @__PURE__ */ new Date()) {
...
// src/reports/contradictions.js
function contradictions(graph, config = {}) {
  const rows = [];
  ...
  for (const record of graph.records) {
    if ((claim.statuses ?? []).includes(record.implementationStatus) && Number(record.source.__fileSize ?? 0) < Number(claim.minimum_bytes ?? 0)) {
      rows.push({
        type: "implementation-claim-stub",
        id: identity(record),
        label: label(record),
        path: record.path,
        detail: `${record.implementationStatus} \xB7 ${record.source.__fileSize ?? 0}b`
      });
    }
  ...
  for (const subject of subjects(graph)) {
    ...
    if (count >= Number(subjectRule.minimum_artifacts ?? Infinity) && Number(subject.source.__fileSize ?? 0) < Number(subjectRule.minimum_bytes ?? 0)) {
      rows.push({
        type: "populated-subject-stub",
        id: identity(subject),
        label: label(subject),
        path: subject.path,
        detail: `${count} docs \xB7 index ${subject.source.__fileSize ?? 0}b`
      });
    }
  }
...
// src/renderers/obsidian.js
function renderObsidian(report, container) {
  container.empty?.();
  const heading = document.createElement("h2");
  heading.textContent = report.report;
  container.appendChild(heading);
  if (report.summary) {
    const list2 = document.createElement("ul");
    for (const [key, value2] of Object.entries(report.summary)) {
      const item2 = document.createElement("li");
      item2.textContent = `${key}: ${value2}`;
  ...
  for (const row of rows) {
    ...
    for (const key of headers) tr.insertCell().textContent = typeof row[key] === "object" ? JSON.stringify(row[key]) : String(row[key] ?? "");
  }
  container.appendChild(table);
  return container;
}

// src/adapters/dataview.js
function dataviewPages(pages) {
  return [...pages].map((page) => ({ ...page, sourceId: page.file?.path, path: page.file?.path }));
}
function dataviewAdapter(dv, scope) {
  return dataviewPages(dv.pages(scope));
}

// src/browser.js
function parseProfileYaml(source) {
  return browser_default.parse(source);
}
function lintEstate(graph, options = {}) {
  return [...lintCore(graph), ...lintProject(graph), ...options.diagnostics === false ? [] : lintDiagnostics(graph, options.now ?? /* @__PURE__ */ new Date())];
}
export {
  REPORTS,
  buildGraph,
